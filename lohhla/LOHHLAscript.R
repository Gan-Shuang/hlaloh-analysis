...
create.kmer.file <- function(workDir, kmerSize, HLAfastaLoc){
  ...
  setwd(workDir)
  cmd <- paste('MAIN/jellyfish-1.1.10/bin/jellyfish count -m ', kmerSize, ' -s 100M -t 5 ', HLAfastaLoc, sep = '')
  system(cmd)
  cmd <- paste('MAIN/jellyfish-1.1.10/bin/jellyfish dump mer_counts_0 > mer_counts_dumps.fa', sep = '')
  system(cmd)
  cmd <- paste('grep --invert-match \\> mer_counts_dumps.fa > ', kmerSize, 'mer_uniq', sep = '')
  system(cmd)
  system('rm mer_counts_0')
...
if(mapping.step){
  ...
  novoindexCMD <- paste('MAIN/novocraft/novoindex ', workDir, '/', full.patient, '.patient.hlaFasta.nix', ' ', workDir, '/', full.patient, '.patient.hlaFasta.fa', sep = '')
